...
const CSS_DASAR = `
  * { box-sizing: border-box; }
  ...
  table.tabel-cetak.tabel-8355 { font-size: 7.5pt; }
  table.tabel-cetak.tabel-8355 th, table.tabel-cetak.tabel-8355 td { padding: 2px 4px; white-space: nowrap; }
...
// Definisi kolom Formulir 8355 (Daftar Calon Peserta Ujian) — dipakai
// bersama oleh halaman 8355.jsx (tampilan tabel di layar) dan fungsi cetak
// bukaCetak8355 di bawah, supaya urutan & isi kolom selalu konsisten.
...
/**
 * Cetak Formulir 8355 — Daftar Calon Peserta Ujian, format resmi dengan
 * kop surat pemerintahan lengkap (Kabupaten/Dinas/Kecamatan) dan seluruh
 * kolom biodata peserta sesuai standar Dapodik.
 *
 * @param {object}      opsi
 * @param {object|null} opsi.profil        - hasil ambilProfilUntukCetak()
...
export function bukaCetak8355({ profil, siswaList, tahunPelajaran }) {
  const theadHtml = `<tr>${KOLOM_8355.map(([label]) => `<th>${escapeHtml(label)}</th>`).join('')}</tr>`
  const tbodyHtml = siswaList
    .map((s, i) => {
      const sel = KOLOM_8355.map(([, get]) => {
        const val = get(s, i)
  ...
  const ttdHtml = `<div class="ttd-wrapper">${blokTandaTanganHtml(profil, { jabatan: 'Kepala Sekolah' })}</div>`
  ...
  const html = `
    ...
    <body>
      ...
      <p class="sub-judul">${escapeHtml(tahunPelajaran ? `Tahun Pelajaran ${tahunPelajaran}` : '')}</p>
      <table class="tabel-cetak tabel-8355">
        <thead>${theadHtml}</thead>
        <tbody>${tbodyHtml || `<tr><td colspan="${KOLOM_8355.length}" style="text-align:center;color:#888;">Belum ada data.</td></tr>`}</tbody>
      </table>
      ${ttdHtml}
      <script>
        window.onload = function () { window.print(); };
