...
const SklPrintTemplate = React.forwardRef(function SklPrintTemplate(
  ...
  return (
    // PERBAIKAN PENTING: kembali pakai `minHeight` (BUKAN `height`).
    // `height: 297mm` yang kemarin dipasang adalah tinggi KAKU — kalau
    // konten ternyata sedikit lebih tinggi dari 257mm (297mm - 40mm
    // padding), isinya "tumpah" keluar batas box tanpa terkendali,
    // sehingga (a) tetap terpotong ke halaman 2, dan (b) bingkai yang
    // mengikuti tinggi box jadi terlihat terpotong di bagian yang
    // tumpah. `minHeight` adalah tinggi LANTAI: box tidak akan lebih
    // pendek dari 1 halaman A4 (sehingga marginTop:"auto" tetap punya
    // ruang untuk mendorong tanda tangan ke bawah saat konten pendek),
    // tapi box BOLEH membesar mengikuti konten kalau memang lebih
    // panjang dari 1 halaman — jadi tidak ada lagi konten yang terpotong
    // paksa atau bingkai yang robek.
    <div
      ref={ref}
    ...
    >
      {/* BINGKAI: garis ganda (double border) mengelilingi seluruh
          halaman, gaya umum surat keterangan/ijazah resmi. Jarak dari
          tepi kertas diatur lewat padding "8mm" di container luar tadi.
          Div bingkai ini juga flex column + flex:1 supaya tingginya
          mengisi penuh sisa minHeight dari container luar, sehingga
          marginTop:"auto" pada blok tanda tangan tetap bisa mendorongnya
          ke dasar AREA DI DALAM BINGKAI. */}
      <div
        style={{
          flex: 1,
          display: "flex",
      ...
      >
        ...
        {/* marginTop:"auto" memakai sisa ruang kosong di dalam bingkai
            (flex column, flex:1) untuk mendorong blok ini ke dasar area
            bingkai. pageBreakInside "avoid" memastikan blok ini tidak
            pernah terpotong sendiri kalau suatu saat kontennya melebihi
            1 halaman. */}
        <div
          style={{
            textAlign: "right",
